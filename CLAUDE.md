...
- `docs/keys-system.md` — keys/escape-hatch design (12 keys per match distributed across chests by tier weight, 3 keys per player to escape; see `BALANCE.keys`). **Hidden behind `HIDDEN_FEATURES.keys` since 2026-06-11 — escape is now gated by the Console system** (per-player trio of `map.consoleSpots` from the Tiled `Consoles` layer, 3-idle-turn channel per console, consoles stay dark for the first `BALANCE.consoles.activationDelayTurns` turns (~30s), `BALANCE.consoles`, resolver step 9.45; client polish — dotted nav path to the next console, door-style fog memory, completion mini-flare — lives in `MatchScene`/`MapRenderer`; full gate inventory in `HIDDEN_STUFF.md` §3)
